...
<head>
  ...
  <title>WMS — {{ isset($inbound) ? 'Edit' : 'Tambah' }} Incoming</title>
...
<body>
  ...
  <main class="container mt-4">
    <h1 class="h4 fw-bold mb-4">{{ isset($inbound) ? 'Edit' : 'Tambah' }} Incoming</h1>
    ...
    <div class="card shadow-sm">
      <div class="card-body p-4">
        <form action="{{ isset($inbound) ? route('inbound.update', $inbound->id) : route('inbound.store') }}" method="POST">
          @csrf
          @if(isset($inbound)) @method('PUT') @endif
          
          <div class="row g-3">
            <div class="col-md-4">
              <label class="form-label">ID Incoming</label>
              <input type="text" name="incoming_id" class="form-control @error('incoming_id') is-invalid @enderror" 
                     value="{{ old('incoming_id', $inbound->incoming_id ?? '') }}" placeholder="INC-001" required>
            ...
            <div class="col-md-4">
              ...
              <input type="date" name="date_received" class="form-control @error('date_received') is-invalid @enderror" 
                     value="{{ old('date_received', isset($inbound) ? $inbound->date_received->format('Y-m-d') : '') }}" required>
            ...
            <div class="col-md-4">
              ...
              <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                <option value="Pending" {{ old('status', $inbound->status ?? 'Pending') == 'Pending' ? 'selected' : '' }}>Pending</option>
                <option value="Diterima" {{ old('status', $inbound->status ?? '') == 'Diterima' ? 'selected' : '' }}>Diterima</option>
                <option value="Ditolak" {{ old('status', $inbound->status ?? '') == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
              </select>
              @error('status') <div class="invalid-feedback">{{ $message }}</div> @enderror
              <small class="text-muted">Catatan: Ubah status ke "Diterima" akan otomatis menambah stok barang</small>
            </div>
            <div class="col-12">
